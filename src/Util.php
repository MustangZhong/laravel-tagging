...
class Util implements TaggingUtility
{
    ...
    public static function slug($name)
    {
        ...
        // For Chinese, Covert to pinyin
        try {
            $str = app()->make(Pinyin::class)->permlink($str, $options['delimiter']);
        } catch (\Exception $e) {
            \Log::warning('The package [Overtrue\Pinyin] is missing');
        }

        $uniqueTagName = config('taggable.unique_tag_name', false);
        if ($uniqueTagName) {
            $model = config('tagging.tag_model', '\Conner\Tagging\Model\Tag');
            $tag = $model::where('slug', $str)->first();
            if ($tag && strcmp($tag->name, $str) !== 0) {
